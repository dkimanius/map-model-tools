...
def sample_from_map(grid_values, origin, voxel_size, coordinates, padding_mode='border'):
    """
    Sample values from a 3D map using provided coordinates.

    Args:
    - grid_values (torch.Tensor): A 3D grid of values to sample from (size: [D, H, W]).
    - origin (float): The origin of the grid in world coordinates.
    - voxel_size (float): The size of each voxel in the grid.
    - coordinates (torch.Tensor): The 3D coordinates where sampling should occur (size: [N, 3]).

    Returns:
    - sampled_values (torch.Tensor): The values sampled from the grid at the input coordinates (size: [N]).
    """

    Z, Y, X = grid_values.shape
    c = (coordinates - origin) / voxel_size
    c[:, 0] = 2 * c[:, 0] / X - 1
    c[:, 1] = 2 * c[:, 1] / Y - 1
    c[:, 2] = 2 * c[:, 2] / Z - 1

    # Fix dimensionality for grid_sample call
    c = c.view(1, 1, 1, -1, 3)
    grid_values = grid_values.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions

    # Use grid_sample to perform interpolation
    sampled_values = torch.nn.functional.grid_sample(
        grid_values, c, mode='bilinear', padding_mode=padding_mode)

    return sampled_values.flatten()
